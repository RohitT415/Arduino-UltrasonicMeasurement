...
void auto_avoidance(){
 centerscanval = watch(); //assigns the watch method to a int
 if (centerscanval>distancelimit) //if objects are not within the set distance, continue moving forward
 { 
  set_Motorspeed(FAST_SPEED,SPEED);
      go_Advance();
 }
 else  //when the ultrasonic sensor detects objects within the set distance, stop and beep
  if(centerscanval<distancelimit)
  {
    stop_Stop();
    alarm();
